...
CREATE TABLE apps.session_token (
  session_id      BIGINT IDENTITY(1,1) CONSTRAINT pk_session_token PRIMARY KEY,
  user_id         BIGINT NOT NULL,
  token           NVARCHAR(512) NOT NULL, -- JWT or opaque token
  issued_at       DATETIME2(3) NOT NULL CONSTRAINT df_session_issued_at DEFAULT (SYSUTCDATETIME()),
...
CREATE INDEX ix_session_user ON apps.session_token(user_id, expires_at);
GO

CREATE TABLE apps.refresh_token (
  refresh_id      BIGINT IDENTITY(1,1) CONSTRAINT pk_refresh_token PRIMARY KEY,
  user_id         BIGINT NOT NULL,
...
CREATE INDEX ix_refresh_user ON apps.refresh_token(user_id, expires_at);
GO
